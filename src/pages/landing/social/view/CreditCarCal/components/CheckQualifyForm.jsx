...
const CheckQualifyForm = ({ handleNextStep, currentStep }) => {
  ...
  const validateForm = () => {
    ...
    if (formData.spending === '' || isNaN(formData.spending) || parseFloat(formData.spending) < 0) {
    ...
    <div className="py-6 space-y-6 max-w-lg mx-auto">
      {selectOptions.map((option) => (
        <div key={option.id} className="space-y-1">
          ...
          />
          {errors[option.id] && <p className="text-red-500 text-sm">{errors[option.id]}</p>}
        </div>
      ))}

      <article className="text-md space-y-2 text-gray-600 w-full bg-blue-50 p-4 rounded-lg border border-blue-100">
        <p>
          <strong>APR:</strong> Annual Percentage Rate - the yearly interest rate charged on your
          credit card balance.
        </p>
        <p>
          <strong>Minimum Repayment:</strong> The smallest amount you must pay monthly on your
          credit card.
        </p>
      </article>
